    ...
    <UCard class="w-full h-min overflow-clip !z-50 relative" :ui="{ body: { padding: '' } }">
      <div class="flex flex-col">
        ...
        <textarea name="content" id="content" placeholder="Take a note..."
          class="p-3 outline-none bg-transparent resize-none transition-all" :rows="textareaRows"
          v-model="noteFormStore.content" @focus="handleFocus" @blur="handleBlur"></textarea>
        ...
        <div class="flex justify-between items-center px-3 py-1 border-t border-gray-700">
          ...
          <button
            ...
            :disabled="loading" @click="onSubmit" @focus="handleFocus" @blur="handleBlur">
            Close
          </button>
        </div>
      </div>
...
const saveDebounced = saveDebouncer(save, 1000)
